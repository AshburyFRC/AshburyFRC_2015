...
public class DriveTrain extends Subsystem {
	...
	public void getToBin() {
		...
		while(!atSwitch()){
			...
			NIVision.imaqColorThreshold(initialImage, processedImage, 255, NIVision.ColorMode.HSV, TOTE_HUE_RANGE, TOTE_SAT_RANGE, TOTE_VAL_RANGE);
			...
			int numParticles = NIVision.imaqCountParticles(processedImage, connectivity8);
			if(numParticles > 0){
				...
				for(int particleIndex = 0; particleIndex < numParticles; particleIndex++)
				{
					...
					par.PercentAreaToImageArea = NIVision.imaqMeasureParticle(processedImage, particleIndex, 0, NIVision.MeasurementType.MT_AREA_BY_IMAGE_AREA);
					par.Area = NIVision.imaqMeasureParticle(processedImage, particleIndex, 0, NIVision.MeasurementType.MT_AREA);
					par.ConvexHullArea = NIVision.imaqMeasureParticle(processedImage, particleIndex, 0, NIVision.MeasurementType.MT_CONVEX_HULL_AREA);
					par.BoundingRectTop = NIVision.imaqMeasureParticle(processedImage, particleIndex, 0, NIVision.MeasurementType.MT_BOUNDING_RECT_TOP);
					par.BoundingRectLeft = NIVision.imaqMeasureParticle(processedImage, particleIndex, 0, NIVision.MeasurementType.MT_BOUNDING_RECT_LEFT);
					par.BoundingRectBottom = NIVision.imaqMeasureParticle(processedImage, particleIndex, 0, NIVision.MeasurementType.MT_BOUNDING_RECT_BOTTOM);
					par.BoundingRectRight = NIVision.imaqMeasureParticle(processedImage, particleIndex, 0, NIVision.MeasurementType.MT_BOUNDING_RECT_RIGHT);
					particles.add(par);
				}
				particles.sort(null);
				//Find the largest most bin-shaped particle
				for(int particle = 0; particle < particles.size(); particle++){
					scores.Trapezoid = TrapezoidScore(particles.elementAt(particle));
					SmartDashboard.putNumber("Trapezoid", scores.Trapezoid);
					scores.LongAspect = LongSideScore(particles.elementAt(particle));
					SmartDashboard.putNumber("Long Aspect", scores.LongAspect);
					scores.ShortAspect = ShortSideScore(particles.elementAt(particle));
					SmartDashboard.putNumber("Short Aspect", scores.ShortAspect);
					scores.AreaToConvexHullArea = ConvexHullAreaScore(particles.elementAt(particle));
				...
				//Determine where the particle is in the image then turn and drive so that the particle is in the center
				
			} else {
				System.err.println("No Particles Found!");//No particles found error message
				break;
			}
		}
	}
