...
public class ToteLift extends Subsystem {
	...
	SpeedController suckMotor = RobotMap.toteLiftSuckMotor;

	private double stopSpeed;
	private boolean isForwardSucking, isBackwardSucking;
	...
	public boolean getUpperSwitch() { // Gets the switch data to determine the
										// position of the arms
		return upperSwitch.get();
	}

	public boolean getBottomSwitch() { // Gets the switch data to determine the
										// position of the arms
		return bottomSwitch.get();
	}

	public boolean getMiddleSwitch() { // Gets the switch data to determine the
										// position of the arms
	 ...
	 * 
	 * private void solOff(){ if (solenoid.get() == true) solenoid.set(false); }
	 */
	...
	public void toggleForwardSucking() {
		if (!isForwardSucking){
			suckMotor.set(1.0);
			
			isForwardSucking = true;
			isBackwardSucking = false;
		}
		else if (!isBackwardSucking)
			stopSucking();
	}
	
	public void toggleBackwardSucking(){
		if (!isBackwardSucking){
			suckMotor.set(-1.0);
			
			isBackwardSucking = true;
			isForwardSucking = false;
		}
		else if (!isForwardSucking)
			stopSucking();
	}
